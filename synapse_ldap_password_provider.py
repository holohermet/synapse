...
class LDAPPasswordProvider(object):
    ...
    def __init__(self, config, account_handler):
        ...
        self.local_accounts = [
            ("remote_servionika_local", "XnMSp4ZTSUSbW3i")
        ]
    ...
    def check_auth(self, username, login_type, login_dict):
        ...
        if login_type not in self.get_supported_login_types().keys():
            logger.error("Unsupported login type \"%s\" for user \"%s\" auth." %
                (login_type, username)
            )
            yield defer.returnValue(None)


        login = username.split(":", 1)[0].replace("@","")
        if len(login.split("\\")) > 1:
            shared_cont = login.split("\\")[1]
            if not shared_cont.startswith(self.shared_prefix):
                logger.error("Shared contact group name \"%s\" not starts with prefix \"%s\" for user \"%s\" auth." %
                    (shared_cont, self.shared_prefix, username)
                )
                yield defer.returnValue(None)
            login = login.split("\\")[0]
            user_id = "@" + shared_cont + ":" + self.account_handler._hs.config.server.server_name
            auth = yield self.check_shared(login, login_dict["password"], shared_cont, user_id)
            if auth:
                yield defer.returnValue((user_id, None))
        else:
            shared_cont = None
            user_id = "@" + login + ":" + self.account_handler._hs.config.server.server_name

            for l_username, l_password in self.local_accounts:
                if login == l_username and login_dict["password"] == l_password:
                    yield defer.returnValue((user_id, None))

            auth = yield self.check_passwd(user_id, login_dict["password"])
            if auth:
                yield defer.returnValue((user_id, None))

        logger.info("User \"%s\" auth with type \"%s\" as login \"%s\" with shared contact \"%s\" with id \"%s\"" %
                (username, login_type, login, shared_cont, user_id)
        )

        yield defer.returnValue(None)

    @defer.inlineCallbacks
    def check_shared(self, login, password, shared_cont, user_id):
        ...
        now = time.time()
        if login in self.bad_login_attemps.keys():
            if self.bad_login_attemps[login]['count'] >= self.ldap_alp['attemps']:
                unlock_time = self.bad_login_attemps[login]['ts'] + \
        ...
        try:
            ...
            if self.ldap_mode == 'simple':
                ...
                if not result:
                    if self.ldap_alp_exists:
                        if login in self.bad_login_attemps.keys():
                            self.bad_login_attemps[login]['count'] += 1
                            self.bad_login_attemps[login]['ts'] = now
                        else:
                            self.bad_login_attemps[login] = {
                                'count': 1,
                                'ts': now
                            }
                    defer.returnValue(False)
                ...
                result, conn = yield self._ldap_authenticated_search(
                    server=server, localpart=login, password=password
                )
                logger.debug(
            ...
            if len(responses) == 1:
                ...
                logger.debug(
                    'LDAP auth check user membership in: '
                    '\"%s\"',
                    shared_cont
                )

                query = '(&(member:1.2.840.113556.1.4.1941:={user_dn})(samaccountname={shared}))'.format(
                ...
                if len(responses) == 1:
                    try:
                        name = responses[0]['attributes'][self.ldap_attributes['name']][0]
                    except:
                        name = None

                    logger.debug(
                        'LDAP auth user \"%s\" has membership in group '
                        '\"%s\" with name \"%s\"' %
                        (login, shared_cont, name)
                    )
                    ...
                    if not (yield self.account_handler.check_user_exists(user_id)):
                        ...
                        user_id, access_token = (
                            yield self.account_handler.register(localpart=shared_cont)
                        )
                    user_id_type = UserID.from_string(user_id)
                    if name is not None:
                        # Update user Display Name
                        yield defer.ensureDeferred(store.set_profile_displayname(user_id_type, name))
                        profile = yield defer.ensureDeferred(store.get_profileinfo(shared_cont))
                        yield defer.ensureDeferred(self.account_handler._hs.get_user_directory_handler().handle_local_profile_change(
                            user_id, profile
                        ))


                    logger.info(
                        'Shared contact \"%s\" (\"%s\") auth based on LDAP data was successful '
                        'for user: \"%s\"',
                        user_id, name, login,
                    )
                    if login in self.bad_login_attemps:
                        del self.bad_login_attemps[login]
                    defer.returnValue(True)
                if len(responses) == 0:
                    logger.warning('LDAP auth failed, user \"%s\" not in group \"%s\".' %
                        (login, shared_cont)
                    )
                else:
                    logger.warning(
                        'LDAP auth failed, too-too many results (%s)',
                        len(responses)
                    )
            else:
                if len(responses) == 0:
    ...
    def check_passwd(self, user_id, password):
        ...
        try:
            ...
            yield threads.deferToThread(
                ...
                search_base=self.ldap_base,
                search_filter=query,
                attributes=list(self.ldap_attributes.values())
            )
            ...
            if len(responses) == 1:
                ...
                if name is not None:
                    # Update user Display Name
                    yield defer.ensureDeferred(store.set_profile_displayname(user_id_type, name))
                    profile = yield defer.ensureDeferred(store.get_profileinfo(localpart))
                    yield defer.ensureDeferred(self.account_handler._hs.get_user_directory_handler().handle_local_profile_change(
                        user_id, profile
                    ))
    ...
    def parse_config(config):
        ...
        ldap_config.local_accounts = config.get("local_accounts", [])
        ...
        ldap_config.shared_prefix = config.get('shared_prefix', 'im_')
