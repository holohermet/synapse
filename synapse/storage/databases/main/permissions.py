...
class AccessWorkerStore(SQLBaseStore):
    ...
    async def get_all_user_permissions(self, user_id):
        ...
        if groups:
            for group in groups:
                group_check = await self.db_pool.simple_select_list(
                    table="permissions",
                    keyvalues={
                        "entity": group.get("name"),
                    },
                ...
                for g_check in group_check:
                    group_allow = False
                    ind_allow = False
                    if (not group_allow and not ind_allow) or (
                        group_allow and not ind_allow):
                        permissions.append({
                            "method": g_check.get("method"),
                            "path": g_check.get("path")
                        })
